...
#include "bmp.h"
 ...
 {
    /*k = canale(liv), h = righe(rig), w = colonne(col)*/
	unsigned int liv, rig, col;
    float risNormRand;

    /* controlla se la ip_mat in ingresso sia inizializzata */
    if(t == NULL)
    {
    ...
    for(liv = 0; liv < t -> k; liv++)
    {
        for(rig = 0; rig < t -> h; rig++)
        {
            ...
            {
                /*ottengo il valore*/
                risNormRand = get_normal_random(mean, std);
...
ip_mat * ip_mat_create(unsigned int h, unsigned int w,unsigned  int k, float v)
{
    ...
    /* vedo se Ipmat è stata allocata correttamente */
    if(Ipmat == NULL)
    {
    ...
    if( ( Ipmat -> data = (float***) malloc(k * sizeof(float**)) ) )
    {
        ...
        {
            /*alloca h righe (puntatori)*/
            if( ( Ipmat -> data[liv] = (float**) malloc(h * sizeof(float*)) ) ) 
            {
...
ip_mat * ip_mat_copy(ip_mat * in) {
 ...
 ip_mat * ip_mat_subset(ip_mat * t, unsigned int row_start, unsigned int row_end, unsigned int col_start, unsigned int col_end) {
     ...
     */
     if( (t == NULL) || (row_start > row_end)  ||  (col_start > col_end)  ||  (row_start < zero)  ||  (col_start < zero)  ||  (row_end > t->h)  ||  (col_end > t->w) )
     {
         printf("Errore! IPMAT non valida/e\n");
         exit(1);
     }
...
ip_mat * ip_mat_concat(ip_mat * a, ip_mat * b, int dimensione) {
    ...
    if( (a == NULL || b == NULL) || ( dimensione < 0) || ( dimensione > 2 ) )
    {
        printf("Errore! IPMAT non valida/e\n");
        exit(1);
    }
    ...
    if ( dimensione == 0 ) {
        if ( (a->w == b->w)  &&  (a->k == b->k) ){
            ...
            printf("Errore! DIMENSIONE 0 non valida/e\n");
            exit(1);
        }
    }
    ...
    if ( dimensione == 1 ) {
        ...
        /* ora vado a riempire le righe e colonne e k di data*/
            ...
            printf("Errore! DIMENSIONE 1 non valida/e\n");
            exit(1);
        }
    }
    ...
    if ( dimensione == 2 ) {
        ...
        /* ora vado a riempire le righe e colonne e k di data*/
            ...
            printf("Errore! DIMENSIONE 2 non valida/e\n");
            exit(1);
        }
    }
...
ip_mat *ip_mat_sum (ip_mat *a, ip_mat *b)
{
    ...
    /* controlliamo che tutte le dimensioni siano uguali e che le ip:mat in input siano allocate */
    ...
    out = ip_mat_create(a->h, a->w, a->k, 0);  /*crea ip_mat output*/
...
/* sottrazione di due ip_mat (tutte le dimensioni devono essere uguali) */
...
/* moltiplica uno scalare per una ip_mat */
...
/* Aggiungi uno scalare ad una ip_mat */
ip_mat * ip_mat_add_scalar (ip_mat *a, float c)
{
    ...
    for(liv = 0; liv < out->k; liv++) 
    {
        for(rig = 0; rig < out->h; rig++) 
        {
...
ip_mat * create_gaussian_filter(unsigned int h, unsigned int w, unsigned int k, float sigma)
{
    ...
    /* abbiamo messo 3 perchè sulle specifiche del progetto è scritto che lavoreremo sempre con matrici data contenenti solo 3 canali per rappresentare il range di colori R G B */
    
    somma = (float*)malloc(sizeof(float)*3);
